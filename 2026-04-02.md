...
DNS 서버 변경이 어려운 환경이면 hosts 파일 직접 수정:

```
C:\Windows\System32\drivers\etc\hosts
```

> ⚠️ 메모장을 **관리자 권한**으로 실행 후 편집
...
### vsftpd 시작 실패 (SSL 인증서 없음)
```bash
# pem 파일 존재 확인
ls -la /etc/vsftpd/vsftpd.pem
# 없으면 인증서 재생성
```

### FTP 업로드 553 오류 (권한 문제)
```bash
chmod 755 /home/본인이름/ftp/uploads
chown 본인이름:본인이름 /home/본인이름/ftp/uploads
setsebool -P ftp_home_dir 1
```

### 브라우저에서 도메인 접속 시 엉뚱한 사이트 표시
- 원인: Windows가 공용 DNS로 질의함
- 해결: hosts 파일에 직접 IP-도메인 매핑 추가

### ERR_CONNECTION_REFUSED
- 원인: DNS는 정상인데 Apache가 꺼져있거나 방화벽 문제
```bash
systemctl status httpd
